  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!officeOnlineServer) {
        throw new Error(
          'process.env.OFFICE_ONLINE_SERVER must be defined and point to an instance of Office Online Server',
        );
      }
                  ...
                  if (implemented?.includes(el.attributes.name)) {
                    ...
                    if (el.attributes?.ext) {
                      if (
                        ...
                        data[el.attributes.ext] = [
                          [name, `${splitUrl[0]}?${queryParams}`],
                        ];
                      } else {
                        data[el.attributes.ext].push([
                          name,
                          `${splitUrl[0]}?${queryParams}`,
                        ]);
                      }
                    }
                  }
